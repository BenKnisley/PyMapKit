...
class TextLayer:
    ...
    def draw(self, renderer, cr):
        """ """
        cr.set_source_rgba(*color_converter(self.color), 1)
        ...
        ##
        cr.select_font_face(self.font, slant, boldness)
        cr.set_font_size(self.font_size)
        
        x = 0
        y = 0

        _, _, text_width, text_height, _, _ = cr.text_extents(self.text)

        if self.pos_x == 'c': ## Center
            x = self._MapEngine.width/2 - text_width/2
        elif self.pos_x == 'l': ## Left
            x = self.margin
        elif self.pos_x == 'r': ## Richt
            x = self._MapEngine.width - text_width - self.margin
        else:
            x = self.pos_x
        
        if self.pos_y == 'b': ## Bottom
            y = self._MapEngine.height - text_height/2 - self.margin
        elif self.pos_y == 't': ## Top
            y = text_height + self.margin
        elif self.pos_y == 'c': ## Center
            y = self._MapEngine.height/2 - text_height/2 - self.margin
        else:
            y = self.pos_y
